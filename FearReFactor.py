...
st.markdown("### 🎲 Select Video Fragment" + "  " + "(Recommend: shorter than 15 seconds)")
...
st.write(f'Estimate: {minutes} minutes (Warning: if running locally, GPU is required)')



sec = sec1
fps = 30
MaxCount = (sec2 - sec1)*30
n_images = MaxCount
...
def demo():
    demofiles = os.listdir(r'C:\Users\DrHeh\OneDrive\Documents\Insight Data Science Program\Jupyter python scripts\streamlitPackage\Demo')
    Files = []
    for f in demofiles:
        Files.append(f)
    all_files = pd.DataFrame(Files)
    all_files = all_files.rename(columns={0:"Title"})
    return all_files
...
if st.button('PLAY Demos?'):
    original_path = 'C:/Users/DrHeh/OneDrive/Documents/Insight Data Science Program/Jupyter python scripts/streamlitPackage/Demo'
    selected_path = 'Best Clown Pranks Compilation 2018 (Clown)'
    final_path = original_path + "/" + selected_path + "/"
    playVideo(final_path)
